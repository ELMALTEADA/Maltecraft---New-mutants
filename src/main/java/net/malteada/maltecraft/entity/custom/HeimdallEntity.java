...
public class HeimdallEntity extends HostileEntity implements IAnimatable {
    public HeimdallEntity(EntityType<? extends HostileEntity> entityType, World world) {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heimdall.walk", ILoopType.EDefaultLoopTypes.LOOP));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heimdall.idle", ILoopType.EDefaultLoopTypes.LOOP));
        return PlayState.CONTINUE;
    }
    ...
    public void registerControllers(AnimationData animationData) {
        animationData.addAnimationController(new AnimationController(this, "controller",
                0, this::predicate));
        animationData.addAnimationController(new AnimationController<HeimdallEntity>(this, "attackcontroller",
    ...
    private PlayState attackPredicate(AnimationEvent event) {
        ...
        if(this.handSwinging && event.getController().getAnimationState().equals(AnimationState.Stopped)){
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.heimdall.attack", ILoopType.EDefaultLoopTypes.PLAY_ONCE));
    ...
    public void onDeath(DamageSource source) {
        ...
        if (!this.world.isClient) {
            ...
            ItemStack customDrop = new ItemStack(Items.ROTTEN_FLESH, 4);
            this.dropStack(customDrop);
            ItemStack customDrop2 = new ItemStack(Items.ENDER_PEARL, 1);
            ...
            if (random.nextFloat() < 0.25f) { // 25% chance to drop a golden apple
                ItemStack randomDrop = new ItemStack(Items.FERMENTED_SPIDER_EYE, 1);
                this.dropStack(randomDrop);
            }
        }
    }
    // Name and color name
    private static final Text ENTITY_NAME = Text.of("§cHeimdall");
